...
class CircularSlider extends Component {
  ...
  handlePanResponderMove = ({nativeEvent:{locationX,locationY}}) => {
    const location = this.cartesianToPolar(locationX,locationY);
    this.setState({ location })
    this.props.onValueChange(location);
  }

  handlePanResponderEnd = () => {
    this.props.onValueChangeEnd(this.state.location);
  }
  render(){
    ...
    return (
      <Svg onLayout={this.onLayout} width={width} height={height}>
        <Circle cx={cx} cy={cy} r={r} stroke='#eee' strokeWidth={2.5} fill='none'></Circle>
        ...
        <G x={endCoord.x-7.5} y={endCoord.y-7.5}>
          <Circle cx={7.5} cy={7.5} r={25} fill={meterColor} {...this._panResponder.panHandlers}/>
          <Text key={value+''} x={7.5} y={11.5} fontSize={12} fill={textColor} fontWeight={'bold'} textAnchor="middle">{value+''}</Text>
        </G>
      </Svg>
    )
  }
